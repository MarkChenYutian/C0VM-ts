...
function heapNodeID(ptr: C0Value<"ptr" | "tagptr">, mem: C0HeapAllocator) {
    if (isTagPointerType(ptr)){
        return `heap-${read_ptr(read_tagptr(ptr.value, mem)[0])[0]}`;
    } else if (isPointerType(ptr)){
...
function BFS_heap_scan(state: VM_State, mem: C0HeapAllocator, typedef: Map<SourceType, AliasType>): [Node<VisData>[], Edge<undefined>[]]
{
    ...
    let job_list: {val: C0Value<"ptr"|"tagptr">, lv: number}[] = [];
    job_list = scan_stack_get_ptrs(state);

    const output: Node<VisData>[] = [];
    let edges : Edge<undefined>[] = scan_stack_build_edges(state, mem);
    ...
    while (job_list.length > 0) {
        ...
        if (drawn_ids.has(heapNodeID(val, mem))) { continue; }
        else { drawn_ids.add(heapNodeID(val, mem)); }
        ...
        job_list = [...job_list, ...get_successors(val, mem, state, lv)];
 ...
 */
function get_successors(V: C0Value<"ptr"|"tagptr">, M: C0HeapAllocator, S: VM_State, lv: number): {val: C0Value<"ptr"|"tagptr">, lv: number}[]
{
    ...
    if (isPointerType(V)){
        ...
        switch (V.type.kind) {
            case "arr":
                ...
                for (let i = 0; i < js_arr.length; i ++) {
                    ...
                    } else if (isTagPointerType(arr_c0val) && !isNullPtr(arr_c0val.value)) {
                        output.push({val: remove_tag(arr_c0val, M, S.TagRecord), lv: lv + 1});
                    }
                }
                break;
            case "ptr":
                if (is_struct_pointer(V) && !isNullPtr(V.value)) {
                    ...
                    js_struct.forEach(
                        ({value, offset, name}) => {
                            ...
                            } else if (value !== undefined && isTagPointerType(value) && !isNullPtr(value.value)) {
                                output.push({val: remove_tag(value, M, S.TagRecord), lv: lv + 1});
                            }
                        }
                    )
        ...
        const real_V = remove_tag(V, M, S.TagRecord);
        if (is_struct_pointer(real_V) && !isNullPtr(real_V.value)) {
            const js_struct = expand_C0Struct(M, S.TypeRecord, real_V);
            js_struct.forEach(
                ({value, offset, name}) => {
                    if (value !== undefined && isPointerType(value) && !isNullPtr(value.value)) {
                        output.push({val: value, lv: lv + 1});
            ...
            const js_deref = deref_C0Value(M, real_V);
            if (isPointerType(js_deref) && !isNullPtr(js_deref.value)) {
                output.push({val: js_deref, lv: lv + 1});
            }
        }
 ...
 */
function C0_Value_to_graph(
    v: C0Value<"ptr"|"tagptr">,
    state: VM_State,
    mem: C0HeapAllocator,
    ...
    if (isPointerType(v)) {
        if (is_struct_pointer(v)){
            ...
            for (let i = 0; i < elems.length; i ++) {
                const elem = elems[i];
                if ((isTagPointerType(elem) || isPointerType(elem)) && !isNullPtr(elem.value)) {
                    result_edge.push(edge_factory(
                        heapNodeID(v, mem),
                        heapNodeID(elem, mem),
                        arrSrcHandleID(i)
    ...
    } else if (isTagPointerType(v) && !isNullPtr(v.value)) {
        if (v.type.value.type === "<unknown>"){
            const [, tag] = read_tagptr(v.value, mem);
            const real_type = state.TagRecord.get(tag);
            if (real_type !== undefined) v.type.value = real_type;
        }
        const real_pointer = remove_tag(v, mem, state.TagRecord);
        return C0_Value_to_graph(real_pointer, state, mem, typedef, x, y);
    }

    throw new internal_error("Impossible Branch Reached");
}
 ...
 */
function scan_stack_get_ptrs(state: VM_State): {val: C0Value<"ptr"|"tagptr">, lv: number}[] {
    let result = [];
    ...
    for (let i = state.CallStack.length - 1; i > -1 ; i --) {
        ...
        for (let j = 0; j < frame.V.length; j ++) {
            const val = frame.V[j];
            if (val !== undefined && (isPointerType(val) || isTagPointerType(val)) && !isNullPtr(val.value)) {
 ...
 */
function scan_stack_build_edges(state: VM_State, allocator: C0HeapAllocator): Edge<undefined>[] {
    ...
    for (let i = 0; i < state.CallStack.length; i ++) {
        ...
        for (let j = 0; j < frame.V.length; j ++) {
            let val = frame.V[j];
            if (val !== undefined && (isPointerType(val) || isTagPointerType(val)) && !isNullPtr(val.value)) {
                if (isTagPointerType(val)){
                    val = remove_tag(val, allocator, state.TagRecord);
                }
                result.push(edge_factory(
                    stackNodeID(i),
                    heapNodeID(val as C0Value<"ptr">, allocator),
                    stackSrcHandleID(j),
                ));
            }
        }
    ...
    for (let j = 0; j < state.CurrFrame.V.length; j ++) {
        ...
        if (val !== undefined && isPointerType(val)) {
            result.push(edge_factory(
                ...
                heapNodeID(val, allocator),
                stackSrcHandleID(j)
            ));
        } else if (val !== undefined && isTagPointerType(val)) {
            const ptr_val = remove_tag(val, allocator, state.TagRecord);
            result.push(edge_factory(
                stackNodeID(state.CallStack.length),
                heapNodeID(ptr_val, allocator),
                stackSrcHandleID(j),
            ));
        }
    }
...
function edge_factory(source: string, target: string, sourceHandle: string): Edge<undefined> {
    return {
        ...
        targetHandle: heapNodeTargetHandleID(),
        zIndex: 999,
