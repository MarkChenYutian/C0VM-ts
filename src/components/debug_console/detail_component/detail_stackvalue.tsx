...
> {
    ...
    render_tagptr(v: C0Value<"tagptr">) {
        ...
        if (isNullPtr(ptr)) {
            ...
            return <p className="dbg-evaluate-tabular-content">Tagged Pointer [Tag: <code className="tag">{Type2String(v.type.value, this.props.typedef)}</code>] <span className="dbg-extra-information">NULL</span></p>;
        } else {
            const [addr, offset, ] = read_ptr(ptr);
            return  <p className="dbg-evaluate-tabular-content">Tagged Pointer [Tag: <code className="tag">{Type2String(v.type.value, this.props.typedef)}</code>] 
